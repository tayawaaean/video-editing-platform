...
export default function MySubmissionsPage() {
  ...
  const fetchSubmissions = useCallback(async (showLoading = true) => {
    try {
      ...
      const cacheKey = `submissions:submitter:${user?.supabase_uid || 'unknown'}:${statusFilter || 'all'}`;
  ...
  }, [statusFilter, user?.supabase_uid, getCache, setCache]);

  useEffect(() => {
    if (!user) return;
    
    const cacheKey = `submissions:submitter:${user.supabase_uid}:${statusFilter || 'all'}`;
    const cachedData = getCache<Submission[]>(cacheKey);
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <p className="mt-3 text-lg font-light tracking-wide text-black/70">
          View and manage all your video submissions
        </p>
      </div>
